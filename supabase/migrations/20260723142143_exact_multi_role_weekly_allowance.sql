...
as $$
  ...
  select coalesce(sum(
    case
      ...
      else round((amount::numeric * settings.additional_basis_points) / 10000)::bigint
    end
  ), 0)::bigint
  from ranked_roles
  cross join settings;
...
comment on function private.calculate_weekly_role_allowance(public.app_role[], public.app_role, jsonb, integer) is
  'Pays the highest independent role in full and every other eligible role at the exact configured rate without denomination rounding.';
