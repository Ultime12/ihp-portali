                  ...
                  .map(
                    (item) => `
                      <tr>
                        ...
                        <td>${typeof ihpRoleBadges === "function" ? ihpRoleBadges(item) : esc(roleLabels(item))}</td>
...
function disciplineOperationsPage() {
  ...
  if (!logoReportCanDownload()) {
    return `
      ${pageHeader("Disiplin İşlemleri", "Yetki gerekli", "Bu alan yalnızca disiplin kurulu yetkilileri ve admin tarafından görülebilir.")}
...
async function logoReportExportMemberReport(memberId) {
  if (!logoReportCanDownload()) {
    showToast("Üye raporu yalnızca DK yetkilileri ve admin tarafından indirilebilir.", "error");
    return;
  }
