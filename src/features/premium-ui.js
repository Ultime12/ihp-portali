...
const premiumNavGroups = [
  ["Portal", new Set(["overview", "members", "positions", "committees", "announcements", "regulation", "youth", "agreements", "games", "credit", "finance", "access"])],
...
dashboardPage = function premiumDashboardPage() {
  ...
  const profiles = visibleProfiles(data.profiles || []).filter(
    (profile) =>
      profile.status !== "left" &&
      !profile.credit_test_access &&
      !rolesOf(profile).includes("super_admin")
  );
